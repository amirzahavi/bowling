...
describe('<RollPanel>', () => {
  it('should render 0-10 range of buttons by default with RollButton', () => {    
    const { getAllByRole } = render(<RollPanel disabled={false}></RollPanel>);
    const buttons = getAllByRole('button');
    expect(buttons).toHaveLength(12);
  });

  it('should render 0-x range of buttons with RollButton', () => {    
    const { getAllByRole } = render(<RollPanel disabled={false} maxPins={5}></RollPanel>);
    const buttons = getAllByRole('button');    
    expect(buttons).toHaveLength(7);
  });

  it('should render the index number in the buttons', () => {
    const { getAllByRole } = render(<RollPanel disabled={false} maxPins={1}></RollPanel>);
    const buttons = getAllByRole('button');
